...
{% hint style="info" %}
The default authentication method is **Client Secret**, so existing credentials are unaffected. Switch a credential to **Certificate** only when you want to use one.
{% endhint %}
...
- `private-key.pem`: the RSA private key. Keep it private — you paste it into n8n and it's never sent to Microsoft.
- `certificate.pem`: the public certificate. You upload this to your Entra app registration.

{% hint style="warning" %}
The private key must be an **RSA** key. The client assertion is signed with RS256, so EC and Ed25519 keys aren't supported.
{% endhint %}
...
1. On your Microsoft application page, select **Certificates & secrets** in the left navigation.
1. Select the **Certificates** tab, then select **Upload certificate**.
1. Select your `certificate.pem` file (the public certificate, not the private key).
1. Enter an optional **Description**, such as `n8n credential`, then select **Add**.
...
1. Paste the contents of `private-key.pem` into the **Private Key** field.
1. Paste the contents of `certificate.pem` into the **Certificate** field.
1. If you see other fields in the n8n credential, refer to [Service-specific settings](#service-specific-settings) below for guidance on completing those fields.
1. Select **Connect my account** in n8n to finish setting up the connection.
1. Log in to your Microsoft account and allow the app to access your info.

Token refresh uses the certificate automatically — no client secret is sent at any point.
